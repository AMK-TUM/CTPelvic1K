...
from nnunet.experiment_planning.plan_and_preprocess_task import (
...
class ExperimentPlanner(object):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        ...
        assert isfile(
            os.path.join(self.folder_with_cropped_data, "dataset_properties.pkl")
        ...
        self.plans_fname = join(
            self.preprocessed_output_folder, default_plans_identifier + "_plans_3D.pkl"
        )
    ...
    def get_target_spacing(self):
        ...
        print("say Hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
        return target
    ...
    def determine_postprocessing(self):
        ...
        all_region_keys = [
            ...
            for i in props_per_patient[k]["only_one_region"].keys()
        ]
        ...
        for r in all_region_keys:
            all_results = [
                props_per_patient[k]["only_one_region"][r]
                for k in props_per_patient.keys()
            ]
        ...
        for c in classes:
            ...
            if len(all_num_voxels) > 0:
                min_size_per_class[c] = (
                    np.percentile(all_num_voxels, 1) * MIN_SIZE_PER_CLASS_FACTOR
                )
            else:
        ...
        for c in classes:
            region_sizes = [
                l
                for k in props_per_patient
                for l in props_per_patient[k]["region_volume_per_class"][c]
            ]
            if len(region_sizes) > 0:
                min_region_size_per_class[c] = min(region_sizes)
                # we don't need that line but better safe than sorry, right?
                min_region_size_per_class[c] = min(
                    min_region_size_per_class[c], min_size_per_class[c]
                )
            else:
    ...
    def plan_experiment(self):
        ...
        print('Miiiiiiiiiiiiiiiiiiiiiiiiiii')
        all_classes = self.dataset_properties["all_classes"]
        print('Hiiiiiiiiiiiiiiiiiiiiiiiiiii')
        ...
        print('Piiiiiiiiiiiiiiiiiiiiiiiiiii')
        ...
        if more:
            ...
            while (
                ...
                * architecture_input_voxels_here
                ...
                num_voxels = np.prod(
                    target_spacing / lowres_stage_spacing * median_shape, dtype=np.int64
                )
